...
| `MANGOHUD_PATH`, `VKBASALT_PATH`, `ASSETS_PATH`, `GAMES_PATH` | subdirs of `SCOPEBUDDY_PATH` |
...
- **Gamescope and standalone MangoHud are mutually exclusive** — `enable_gamescope`
  ...
  gamescope, set `MANGOHUD=0` and prepend the wrapper to `command` instead.
- **The MangoHud wrapper on a launcher command blacks out its window** — the
  Minecraft launcher started as `mangohud --dlsym <launcher>` from a game config
  comes up as a black CEF window. An OpenGL game behind such a launcher gets the
  HUD from the profile's plain `MANGOHUD=1` alone, wrapper-free.
- **`SCB_PRE_COMMAND` reaches the gamescope path only** — the launcher evaluates
  it immediately before it execs gamescope, so a config running with
  `SCB_NOSCOPE=1` — the state every profile default applies — never sees it. A
  command that must run before the game starts belongs in the game config body,
  which both paths source (see `games/lutris/minecraft.conf`).
  `SCB_POST_COMMAND` sits on an `EXIT` trap and runs on both paths.
...
The launcher, both Proton builds and the Vulkan layers read these; a game config
sets them with a plain `export`.

| Variable | Read by | Effect |
|---|---|---|
| `SCB_PRE_COMMAND` | launcher | Shell snippet evaluated before gamescope starts, on the gamescope path only |
| `SCB_POST_COMMAND` | launcher | Shell snippet evaluated when the launcher exits, on both paths |
...
| `ENABLE_VKBASALT` / `DISABLE_VKBASALT` | Vulkan loader | Turn the vkBasalt post-processing layer on / off for the game |
| `VKBASALT_CONFIG_FILE` | vkBasalt | Effect config to load; the presets live in `vkbasalt/` |
